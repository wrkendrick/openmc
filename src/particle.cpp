...
void Particle::event_advance()
{
  ...
  } else if (model::materials[material()]->continuous_num_density_) {
    collision_distance() = sampling_cvmt(this, boundary().distance);
  } else {
...
// cvmt implementation 
//==============================================================================
double Particle::sampling_cvmt(Particle *p, double d_boundary)
{
  ...
  if(prn(current_seed()) <= PNC) {   //! no collision
    d_collision = INFINITY;
  } else {             //! collision   
    //! sample optical depth 
    tau_hat = -std::log(1.0 - (1.0 - PNC) * prn(current_seed()));
    ...
    xyz_orig = p->coord(p->n_coord()-1).r;
    //! cross sections are updated for later tallying of keff and user tallies
    move_particle_coord(p->coord(p->n_coord()-1), d_collision);
    model::materials[material()]->calculate_xs(*p);
    //! move particle back
    p -> coord(p -> n_coord()-1).r  = xyz_orig;
  }
...
void Particle::move_particle_coord(LocalCoord &coord, double ds)
{
  coord.r += ds*coord.u;
  return;
}

void Particle::copy_data(LocalCoord &to, LocalCoord from)
{
  to.r = from.r;
  to.u = from.u;
  to.cell = from.cell;
  ...
  to.lattice_i[0] = from.lattice_i[0];
  to.lattice_i[1] = from.lattice_i[1];
  to.lattice_i[2] = from.lattice_i[2];
  to.rotated = from.rotated;
  return;
}

void Particle::simpsons_path_integration(double &optical_depth, double distance, std::vector<double> &xs_t, bool dbg_file, int it_num)
{
  double ds;
  double new_temp;
  std::string format;
  ...
  FILE *fp;
  //
  copy_data(coord, this -> coord(this -> n_coord() - 1));
  //
  ds = distance / (double)(settings::cvmt_intervals);
  // 
  if (dbg_file) {
    sprintf(fname, "particle_%5d_it_%5d.out" , (int)this->id(), it_num);
    printf("Opening file %s", fname);
    fp = std::fopen(fname,"w");
    fprintf(fp, "%15.7f %15.7f %15.7f %15.7f %15.7f \n",  
            this -> coord(this -> n_coord()-1).u[0], 
            this -> coord(this -> n_coord()-1).u[1], 
            this -> coord(this -> n_coord()-1).u[2], 
            this -> E(), ds);
  }
  ...
  for(i=1; i<=(settings::cvmt_intervals+1); i++){ 
    ...
    xs_t[i-1] = macro_xs().total;  
    if(dbg_file) {
      printf("String being written to file %s\n :", fname);
      printf("%15.7f %15.7f %15.7f %15.7f %15.7f \n", 
              this -> coord(this -> n_coord()-1).r[0],  
              this -> coord(this -> n_coord()-1).r[1], 
              this -> coord(this -> n_coord()-1).r[2], 
              new_temp, xs_t[i-1]);
      fprintf(fp, "%15.7f %15.7f %15.7f %15.7f %15.7f \n", 
              this -> coord(this -> n_coord()-1).r[0],  
              this -> coord(this -> n_coord()-1).r[1], 
    ...
    move_particle_coord(this -> coord(this -> n_coord()-1), ds);
  }
  //
  copy_data(this -> coord(this -> n_coord()-1), coord);
  //
  optical_depth = 0.0; 
...
void Particle::estimate_flight_distance(std::vector<double> xs_t, double distance, double tau_hat, double &s)
{
  double ds;
...
double Particle::sign(double aa, double bb)
{
  if(bb>1e-50) { 
    return aa;
  } else if (bb<-1e-50) {
...
void Particle::get_quadratic_root(double a, double b, double c, double lower_b, double upper_b, double &root)
{
  //
  double q;
  double C_1, C_2;

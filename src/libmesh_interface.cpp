...
double sample_impl(const libMesh::Point& p)
{
  ...
  sample_count++;
  if (LEACS_DEBUG_MODE && sample_count % 50000 == 0) {
...
void load_solution(const std::string& filepath, const std::string& meshpath,
                   const std::string& sys_name, const std::string& var_name)
{
